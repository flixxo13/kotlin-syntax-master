...
interface LearningState {
  ...
  updateCustomTask:  (task: Exercise) => void;
  ...
  reconcileDecks:    () => void;
  moveTasksToDeck:   (taskIds: string[], targetDeckId: string) => void;
  removeTaskFromDeck:(taskId: string, deckId: string) => void;
...
export const useLearningStore = create<LearningState>()(
  persist(
    (set, get) => ({
      ...
      exerciseNotes:        {},
      spaces:               [
        { id: 'system', title: 'Kotlin Core', description: 'Die offiziellen Grundlagen.', icon: 'BookOpen', type: 'system', order: 0, color: '#7f52ff' },
        { id: 'custom', title: 'Meine Decks', description: 'Importierte und eigene Aufgaben.', icon: 'Download', type: 'custom', order: 1, color: '#ec4899' },
      ],
      decks:                [],
      activeTopicId:        null,
      ...
      updateCustomTask: (task) => set((state) => ({
        customTasks: state.customTasks.map(t => t.id === task.id ? task : t)
      })),
      removeCustomTask:  (id)  => set(s => ({ customTasks: s.customTasks.filter(t => t.id !== id) })),
      clearCustomTasks:  ()    => set({ customTasks: [] }),
      
      // ── Space & Deck Actions ──────────────────────────────────────────────
      addSpace: (space) => set(s => ({ spaces: [...s.spaces, space].sort((a,b) => a.order - b.order) })),
      ...
      reconcileDecks: () => set((state) => {
        // If we have customTasks but no decks, create a default deck
        if (state.customTasks.length > 0 && state.decks.length === 0) {
          const defaultDeck: Deck = {
            id: 'default-import',
            spaceId: 'custom',
            title: 'Meine Importe',
            description: 'Alle bisher importierten Aufgaben.',
            icon: 'Download',
            exercises: state.customTasks,
      ...
      moveTasksToDeck: (taskIds, targetDeckId) => set((state) => {
        const tasksToMove = state.customTasks.filter(t => taskIds.includes(t.id));
        if (!tasksToMove.length) return state;

        const newDecks = state.decks.map(deck => {
          if (deck.id === targetDeckId) {
            const existingIds = new Set(deck.exercises.map(e => e.id));
            const toAdd = tasksToMove.filter(t => !existingIds.has(t.id));
            return { ...deck, exercises: [...deck.exercises, ...toAdd] };
          } else {
            return { ...deck, exercises: deck.exercises.filter(e => !taskIds.includes(e.id)) };
          }
        });

        return { decks: newDecks };
      }),
      removeTaskFromDeck: (taskId, deckId) => set((state) => ({
        decks: state.decks.map(deck =>
          deck.id === deckId
            ? { ...deck, exercises: deck.exercises.filter(e => e.id !== taskId) }
            : deck
        )
      })),
