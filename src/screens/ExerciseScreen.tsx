...
// ★ NEU: Case-Toleranz-Check – erkennt ob Lösung nur Groß/Kleinschreibung abweicht
function getCaseMistake(code: string, solution: string): string | null {
  ...
  if (n(code).toLowerCase() === n(solution).toLowerCase() && n(code) !== n(solution)) {
    // Finde das erste abweichende Token
    const ct = tokenize(code), st = tokenize(solution);
...
// ★ NEU: Chip-Leiste mit horizontalem Scroll + Fade-Rändern
function ChipBar({ chips, solToks, pct }: { chips: any[]; solToks: number; pct: number }) {
  return (
    <div style={{ position: "relative", minHeight: 36, flexShrink: 0, background: "#0a0a10", borderTop: "1px solid #1e1e2e" }}>
      {/* Fade links */}
      ...
      {/* Fade rechts */}
      <div style={{ position: "absolute", right: solToks > 0 ? 60 : 0, top: 0, bottom: 0, width: 16, background: "linear-gradient(to left, #0a0a10, transparent)", zIndex: 2, pointerEvents: "none" }} />

      <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 16px", overflowX: "auto", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" } as any}>
        {chips.length === 0
          ...
          : chips.map((c, i) => {
              ...
              return (
                <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 2, padding: "3px 9px", borderRadius: 6, fontSize: 12, fontFamily: "monospace", fontWeight: 600, background: st.bg, border: `1px solid ${st.border}`, color: st.color, flexShrink: 0, whiteSpace: "nowrap" }}>
      ...
      {/* Progress badge rechts */}
      {solToks > 0 && chips.length > 0 && (
        <div style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 5, zIndex: 3, background: "#0a0a10", paddingLeft: 4 }}>
          <div style={{ width: 36, height: 3, background: "#1e1e2e", borderRadius: 2, overflow: "hidden" }}>
...
export function ExerciseScreen() {
  ...
  // ★ NEU: Mini-Task-Anker im Fullscreen
  const [taskAnchorVisible, setTaskAnchorVisible] = useState(false);

  // ★ NEU: Flash-Animation für zuletzt eingefügtes Token
  ...
  useEffect(() => {
    ...
    const onVvResize = () => {
      ...
      if (!isOpen) {
        ...
        if (rawKbH < 50) { isOpen = false; lockedH = 0; setKbOpen(false); setKbPadding(0); dbg("KB UNLOCKED & CLOSED"); }
      }
    };
    const onWinResize = () => {
      const newBase = window.innerHeight;
      if (Math.abs(newBase - baseHeight) > 50) { baseHeight = newBase; isOpen = false; lockedH = 0; setKbPadding(0); dbg(`BASE → ${baseHeight}`); onVvResize(); }
    };
  ...
  useEffect(() => {
    ...
    setCaseHint(null); setTaskAnchorVisible(false);
  }, [ex]);
      ...
      <>
        ...
        <div style={{
          ...
          <div style={{ height: 46, flexShrink: 0, background: "#0d0d14", borderBottom: "1px solid #1e1e2e", display: "flex", alignItems: "center", padding: "0 10px", gap: 6 }}>
            ...
            {/* Progress + Titel + ★ Mini-Task-Anker */}
          ...
          {/* ★ Mini-Task-Anker Dropdown */}
          {taskAnchorVisible && (
            <div style={{ flexShrink: 0, background: "#0d0d14", borderBottom: "1px solid #252540", padding: "8px 14px", animation: "slideDown 0.15s ease" }}>
              {ex.initialCode && (
                <div style={{ fontFamily: "monospace", fontSize: 11, color: "#6b7280", background: "#111118", borderRadius: 6, padding: "4px 10px", marginBottom: 6, border: "1px solid #1e1e2e" }}>
                  {ex.initialCode}
                </div>
              )}
          ...
          {/* ★ Case-Hinweis Banner */}
          {fb === "case" && caseHint && (
            <div style={{ flexShrink: 0, background: "rgba(251,191,36,.1)", borderBottom: "1px solid rgba(251,191,36,.3)", padding: "6px 14px", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <div style={{
            ...
            <textarea
              ref={taRef}
              value={code}
              onChange={e => { setCode(e.target.value); setCaseHint(null); }}
