...
const ONBOARDING_STEPS = [
  {
    emoji: "✍️",
    title: "Editor",
    body: "Tippe hier deinen Kotlin-Code. Du kannst jederzeit tippen – der Editor ist dein Lernbereich.",
  },
  {
    emoji: "▶ CHECK",
    title: "Code prüfen",
    body: "Drücke CHECK wenn du fertig bist. Grün = richtig, Rot + Shake = noch nicht ganz.",
    highlight: "check",
  },
  {
    emoji: "💡",
    title: "Hints – kontrolliert einsetzen",
    body: "3 gestufte Hinweise stehen bereit. Jeder Hint kostet XP – nutze sie bewusst für nachhaltiges Lernen.",
    highlight: "hint",
  },
  {
    emoji: "◼◻◻",
    title: "Syntax-Feedback",
    body: "Die Chip-Leiste zeigt Token-für-Token ob dein Code stimmt. Du kannst sie vor dem Start deaktivieren – dann wird's schwerer!",
    highlight: "chips",
  },
  {
    emoji: "🔲",
    title: "Vollbild-Modus",
    body: "Tippe auf den Editor-Bereich um in den Vollbild-Modus zu wechseln. Dort stehen dir Shortcut-Buttons zur Verfügung.",
  },
  {
    emoji: "🔁",
    title: "Für später",
    body: "Du entscheidest selbst wann du eine Aufgabe erneut übst. Dein Fortschritt und XP bleiben gespeichert.",
  },
];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 5, justifyContent: "center", marginBottom: 22 }}>
          {ONBOARDING_STEPS.map((_, i) => (
            <div key={i} style={{
              ...
              background: i === step ? "#7c3aed" : i < step ? "#4b3f6b" : "#252540",
              transition: "all .25s",
            }} />
          ))}
        </div>
        ...
        <div style={{
          ...
          color: "#7c3aed", marginBottom: 10, letterSpacing: ".02em",
        }}>{s.emoji}</div>

        {/* Title */}
        <div style={{ fontSize: 18, fontWeight: 700, color: "#f1f0fb", marginBottom: 10 }}>
          {s.title}
        </div>

        {/* Body */}
        <div style={{ fontSize: 14, lineHeight: 1.65, color: "#9ca3af", marginBottom: 28 }}>
          {s.body}
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {step > 0 && (
            <button
              ...
              style={{ flex: 1, height: 48, borderRadius: 12, background: "#1e1e2e", border: "1px solid #2a2a42", color: "#6b7280", fontSize: 14, fontWeight: 700, cursor: "pointer", outline: "none" }}
            >
              ← Zurück
            </button>
          )}
          <button
            onClick={() => { if (isLast) { localStorage.setItem(ONBOARDING_KEY, "1"); onDone(); } else setStep(s => s + 1); }}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: "#f1f0fb", marginBottom: 6 }}>
          Syntax-Feedback aktivieren?
        </div>
        <div style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.6, marginBottom: 20 }}>
          Die Chip-Leiste gibt Token-für-Token-Rückmeldung. Deaktiviere sie für eine größere Herausforderung.
        </div>

        <div style={{ display: "flex", gap: 10 }}>
          <button
            onClick={() => onChoice(false)}
            style={{ flex: 1, height: 48, borderRadius: 12, background: "#1e1e2e", border: "1px solid #2a2a42", color: "#6b7280", fontSize: 13, fontWeight: 700, cursor: "pointer", outline: "none" }}
          >
            ◻ Ohne Feedback
          </button>
          <button
            onClick={() => onChoice(true)}
            style={{ flex: 1, height: 48, borderRadius: 12, background: "rgba(124,58,237,.2)", border: "1px solid rgba(124,58,237,.4)", color: "#c4b5fd", fontSize: 13, fontWeight: 700, cursor: "pointer", outline: "none" }}
          >
            ◼ Mit Feedback
          </button>
        </div>
...
function HighlightTask({ text }: { text: string }) {
  ...
  return <>{parts.map((part, i) =>
    part.startsWith('"') || part.startsWith("'") || part === "read-only"
...
export function ExerciseScreen({ topicId, conceptId, exerciseId, onBack }: ExerciseScreenProps) {
  ...
  // Task overlay: starts OFF in fullscreen (user already read it in normal view)
  ...
  // Onboarding & chip-mode sheet
  const [showOnboarding, setShowOnboarding] = useState(false);
  const [showChipSheet, setShowChipSheet]   = useState(false);

  // vpH: exact visible area, updated on every visualViewport resize (fixes frozen screen / keyboard gap)
  ...
  // ── Viewport height tracking (fixes frozen/gap on Chrome/Samsung) ─────────────
  useEffect(() => {
    ...
    const onResize = () => {
      const h = vv.height;
      // Always sync container height → eliminates keyboard gap
      setVpH(h);
      const kbH = Math.max(0, base - h);
      if (!kbIsOpen && kbH > 100) { kbIsOpen = true;  setKbOpen(true); }
    ...
    // FIX: also re-sync vpH on mount/remount after back-navigation
    setVpH(vv.height);
  ...
  useEffect(() => {
    ...
    setCaseHint(null); setTaskOverlay(false);
  ...
  const requestHint = useCallback(() => {
    ...
    const msg = nextLevel === 99 ? "Lösung eingeblendet" : `Hint ${nextLevel} aktiv 💡`;
    setHintToast(msg);
  ...
  // Opens chip-mode sheet, then fullscreen
  const openFullscreenAndFocus = useCallback(() => {
    setShowChipSheet(true);
  }, []);
  ...
  const enterFullscreen = useCallback((withChips: boolean) => {
    ...
    setFullscreen(true);
    setTaskOverlay(false); // starts hidden — user already read it
      ...
      <>
        ...
        <div style={{ display: "flex", flexDirection: "column", height: vpH, maxWidth: 430, margin: "0 auto", background: "#060609", color: "#f1f0fb", fontFamily: "'Inter',system-ui,sans-serif", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, minHeight: 0, position: "relative", background: "#0a0a10", animation: fb === "incorrect" ? "shake 0.4s cubic-bezier(.36,.07,.19,.97) both" : "none" }}>
            ...
            {/* Floating Task Overlay – draggable */}
            <div style={{ position: "absolute", left: 28, right: 0, top: 0, zIndex: 10, opacity: overlayOpacity, transition: isDraggingOverlay ? "none" : "opacity 0.6s ease", pointerEvents: overlayOpacity < 0.05 ? "none" : "auto", transform: `translate(${overlayPos.x}px, ${overlayPos.y}px)`, willChange: "transform" }}>
              <div style={{ margin: "8px 8px 0", background: "rgba(8,8,16,0.82)", backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderRadius: 10, border: "1px solid rgba(124,58,237,0.25)", padding: "0 0 10px", boxShadow: "0 4px 28px rgba(0,0,0,0.55)", overflow: "hidden" }}>
                <div onPointerDown={onOverlayPointerDown} onPointerMove={onOverlayPointerMove} onPointerUp={onOverlayPointerUp} onPointerCancel={onOverlayPointerUp}
                  style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 12px 6px", cursor: isDraggingOverlay ? "grabbing" : "grab", touchAction: "none", borderBottom: "1px solid rgba(124,58,237,0.12)", background: "rgba(124,58,237,0.06)", userSelect: "none" }}>
          ...
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column" }}>
            ...
            {showTokens ? (
              ...
              >
                {/* Hint Button */}
                <button
                  onTouchEnd={e => { e.preventDefault(); requestHint(); }} onClick={requestHint} disabled={solShown}
                ...
                {/* Check */}
                <button
                  onTouchEnd={e => { e.preventDefault(); check(); }} onClick={check}
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100dvh", maxWidth: 430, margin: "0 auto", background: "#060609", color: "#f1f0fb", fontFamily: "'Inter',system-ui,sans-serif", overflow: "hidden", paddingBottom: 56 }}>
        ...
        {/* Editor Preview */}
        <div style={{ height: 120, flexShrink: 0, position: "relative", background: "#0a0a10", cursor: "pointer", borderBottom: "1px solid #1a1a28" }} onClick={openFullscreenAndFocus}>
          <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 28, background: "#0d0d14", borderRight: "1px solid #1a1a28", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: PAD_T, fontSize: 11, fontFamily: "monospace", color: "#3a3a5a", userSelect: "none", pointerEvents: "none" }}>1</div>
          <div style={{ position: "absolute", inset: 0, left: 28, fontFamily: "'JetBrains Mono',monospace", fontSize: 16, lineHeight: `${LINE_H}px`, color: code ? "#e2e8f0" : "#2a2a3e", padding: `${PAD_T}px 12px 10px`, whiteSpace: "pre-wrap", pointerEvents: "none", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, background: "#0a0a10", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={openFullscreenAndFocus}>
          <span style={{ fontSize: 11, color: "#252540", fontFamily: "monospace", letterSpacing: ".06em" }}>↑ Tippe zum Starten</span>
        </div>
      </div>

      {/* Fixed Bottom Bar – with info labels */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, zIndex: 200 }}>
        <div style={{ background: "#0d0d14", borderTop: "1px solid #1e1e2e", padding: "6px 16px 8px", display: "flex", alignItems: "flex-end", gap: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
            <span style={{ fontSize: 9, fontFamily: "monospace", color: "#374151", letterSpacing: ".04em" }}>Gestufter Hinweis</span>
