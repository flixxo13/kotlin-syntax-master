...
export function ExerciseScreen() {
  ...
  // WICHTIG: kbH als ref statt state - verhindert Re-Render bei Keyboard-Öffnung
  ...
  const focusLockRef = useRef(false); // Verhindert Fokus-Verlust während Öffnung

  // ── visualViewport - verzögertes Update ──
  useEffect(() => {
    ...
    const upd = () => {
      ...
      // Wenn Fokus-Lock aktiv, ignoriere kleine Änderungen
      if (focusLockRef.current && Math.abs(kbHRef.current - newH) < 50) {
        return;
      }
      
      if (Math.abs(kbHRef.current - newH) > 10) {
        kbHRef.current = newH;
        
        // Verzögertes State-Update um Fokus nicht zu stören
  ...
  // ── KRITISCH: Fullscreen öffnen mit Fokus-Lock ──
  const openFullscreenAndFocus = useCallback(() => {
    // Aktiviere Fokus-Lock - verhindert State-Updates während Keyboard öffnet
    focusLockRef.current = true;
    
    setFullscreen(true);
    
    // Warte bis React gerendert hat, dann fokussiere
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        const ta = taRef.current;
        if (ta) {
          ...
          // Halte Fokus-Lock für 500ms aktiv
          setTimeout(() => {
            focusLockRef.current = false;
            // Jetzt erst kbH updaten
