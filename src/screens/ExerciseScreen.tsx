...
export function ExerciseScreen() {
  ...
  useEffect(() => {
    ...
    const upd = () => {
      ...
      setKbH(prev => Math.abs(prev - newH) > 20 ? newH : prev);
    };
  ...
  // ── Fokus Fix für Mobile ──
  const openFullscreenAndFocus = useCallback(() => {
    setFullscreen(true);
    setTimeout(() => {
      if (taRef.current) {
        taRef.current.focus();
        // Verhindert Layout-Sprünge auf iOS
        window.scrollTo(0,0);
      }
    }, 150); // Genug Zeit für die Fullscreen-Transition
  }, []);
  ...
  const TokenBar = () => (
    <div style={{
      ...
      <button onClick={requestHint} disabled={solShown} style={hintBtnStyle(hl, solShown)}>
        ...
        <span onClick={e => { e.stopPropagation(); setSheetH(s => s === null ? 200 : null); }}
          style={{ fontSize: 10, color: "#4b5563", marginLeft: 2, padding: "2px", cursor: "pointer" }}>
          {SHEET_OPEN ? "▾" : "▴"}
        </span>
      </button>
      ...
      <button onClick={check} style={checkBtnStyle(isCorrect)}>
        {isCorrect ? "✓" : "▶"}
      </button>
      ...
      {smartToks.map((s, i) => (
        <button key={i} onClick={() => insert(s)} style={tokenBtnStyle}>{s}</button>
      ))}
    </div>
  );
  ...
  const FallbackBar = () => (
    <div style={{
      ...
      <button onClick={requestHint} style={hintBtnStyle(hl, solShown, true)}>
        ...
        <div style={{ display: "flex", gap: 3 }}>
          {[0, 1, 2].map(i => (
            <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: i < hl ? HCOL[i] : "rgba(255,255,255,.1)" }} />
          ))}
        </div>
      </button>
      <div style={{ flex: 1 }} />
      <button onClick={check} style={largeCheckBtnStyle(isCorrect)}>
        {isCorrect ? "✓" : "▶"} CHECK
      </button>
    </div>
  );
    ...
    return (
      <>
        ...
        <div style={{
          ...
          {/* Header */}
          <div style={{
            height: 46, flexShrink: 0, background: "#0d0d14", borderBottom: "1px solid #1e1e2e",
          ...
          {/* Fixed Accessory Bar over Keyboard */}
          <div style={{ position: "fixed", bottom: kbH, left: 0, right: 0, zIndex: 100 }}>
             {showTokens ? <TokenBar /> : <FallbackBar />}
             {SHEET_OPEN && <div style={{ height: sheetH || 200, background: "#111118", borderTop: "1px solid #2a2a42" }}>
                {/* Drag Handle & Hints ... simplified for space */}
                <div onPointerDown={onSheetDragStart} onPointerMove={onSheetDragMove} onPointerUp={onSheetDragEnd} style={dragHandleStyle} />
                <div style={{ padding: 12, color: "#d1d5db", fontSize: 13, fontFamily: "monospace" }}>
                   {solShown ? "LÖSUNG: " + ex.solution : ex.hints[hl-1] || "Drücke 💡"}
                </div>
             </div>}
          </div>
        </div>
  ...
  return (
    <>
      ...
      <div style={{
        display: "flex", flexDirection: "column",
        height: "100dvh", maxWidth: 430, margin: "0 auto",
        background: "#060609", color: "#f1f0fb", overflow: "hidden",
      }}>
        {/* Header Normal */}
        <div style={{ height: 52, flexShrink: 0, background: "#0d0d14", borderBottom: "1px solid #1e1e2e", display: "flex", alignItems: "center", padding: "0 10px" }}>
        ...
        <div style={{ padding: "12px 14px", background: "#0d0d14", borderBottom: "1px solid #1e1e2e" }}>
           <div style={{ fontSize: 14, color: "#d1d5db" }}>{ex.task}</div>
        </div>

        <ChipsRow />

        {/* Editor Preview (Clickable) */}
        ...
        {/* Static Footer */}
        <div style={{ height: 50, background: "#0d0d14", borderTop: "1px solid #1e1e2e", display: "flex", alignItems: "center", padding: "0 16px" }}>
           <button onClick={requestHint} style={hintBtnStyle(hl, solShown)}>💡</button>
           <div style={{ flex: 1 }} />
           <button onClick={check} style={largeCheckBtnStyle(isCorrect)}>CHECK</button>
        </div>
      </div>
    </>
  );
...
const checkBtnStyle = (isCorrect: boolean) => ({
  ...
  color: isCorrect ? "#4ade80" : "#6b7280", fontSize: 14, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", outline: "none",
});

const largeCheckBtnStyle = (isCorrect: boolean) => ({
  height: 36, padding: "0 20px", borderRadius: 10,
  background: isCorrect ? "linear-gradient(135deg,#16a34a,#22c55e)" : "linear-gradient(135deg,#5b21b6,#7c3aed)",
  border: "none", color: "#fff", fontSize: 13, fontWeight: 700, outline: "none",
});

const tokenBtnStyle = {
  padding: "5px 10px", height: 34, flexShrink: 0, background: "#16162a", border: "1px solid #252540",
  borderRadius: 7, color: "#c4b5fd", fontFamily: "monospace", fontSize: 12, fontWeight: 600, outline: "none",
};

const textareaStyle: any = {
  position: "absolute", inset: 0, left: 28, background: "transparent", border: "none", outline: "none",
  fontFamily: "'JetBrains Mono',monospace", fontSize: "16px", lineHeight: "24px",
  padding: "10px 12px", resize: "none", caretColor: "#7c3aed",
};

const ghostStyle: any = {
  position: "absolute", left: 28, right: 0, padding: "0 12px", pointerEvents: "none",
  fontFamily: "'JetBrains Mono',monospace", fontSize: "16px", lineHeight: "24px", color: "rgba(120,80,220,.25)", whiteSpace: "pre-wrap",
};

const lineNrStyle: any = {
  position: "absolute", left: 0, top: 0, bottom: 0, width: 28, background: "#0d0d14", borderRight: "1px solid #1a1a28",
  display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 10, fontSize: 11, fontFamily: "monospace", color: "#3a3a5a", userSelect: "none",
};

const dragHandleStyle = {
  flexShrink: 0, height: 20, cursor: "ns-resize", display: "flex", alignItems: "center", justifyContent: "center", touchAction: "none",
};

const fbOverlayStyle = (fb: string): any => ({
  position: "absolute", inset: 0, pointerEvents: "none", zIndex: 20, display: "flex", alignItems: "center", justifyContent: "center",
  background: fb === "correct" ? "rgba(34,197,94,.07)" : "rgba(248,113,113,.07)", fontSize: 48,
});
