...
import { QuickEditModal } from "../components/exercise/QuickEditModal";
...
interface ExerciseScreenProps {
  ...
  deckId?:            string;
  onBack?:            () => void;
  onStartCustomTask?: (id: string, deckId?: string) => void;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
export function ExerciseScreen({ topicId, conceptId, exerciseId, deckId, onBack, onStartCustomTask }: ExerciseScreenProps) {
  const { customTasks, decks, rateDifficulty, completeExercise, getRandomCustomTask, updateCustomTask, removeCustomTask } = useLearningStore();

  const EXERCISES: Exercise[] = useMemo(() => {
    if (topicId === "custom") {
      if (deckId) {
        const deck = decks.find(d => d.id === deckId);
        if (deck && deck.exercises.length > 0) {
          const idx = deck.exercises.findIndex(t => t.id === exerciseId);
          return idx <= 0 ? deck.exercises : [...deck.exercises.slice(idx), ...deck.exercises.slice(0, idx)];
        }
      }
  ...
  }, [topicId, customTasks, decks, exerciseId, deckId]);
    ...
    return (
      <>
        ...
        <QuickEditModal 
          ...
          onSave={updateCustomTask}
          onDelete={(id) => {
            removeCustomTask(id);
            if (EXERCISES.length > 1) {
              setExIdx(p => Math.max(0, p - 1));
            } else {
              onBack?.();
            }
          }}
        ...
        <div style={{ display:"flex", flexDirection:"column", height:vpH, maxWidth:430, margin:"0 auto", background:"#060609", color:"#f1f0fb", fontFamily:"'Inter',system-ui,sans-serif", overflow:"hidden" }}>
          ...
          <div style={{ height:46, flexShrink:0, background:"#0d0d14", borderBottom:"1px solid #1e1e2e", display:"flex", alignItems:"center", padding:"0 10px", gap:6 }}>
            ...
            {topicId === "custom" && <button onClick={() => setShowEditModal(true)} style={{ ...toggleBtn(false), fontSize:14 }} title="Bearbeiten">✏️</button>}
      ...
      <div style={{ display:"flex", flexDirection:"column", height:"100dvh", maxWidth:430, margin:"0 auto", background:"#060609", color:"#f1f0fb", fontFamily:"'Inter',system-ui,sans-serif", overflow:"hidden", paddingBottom:56 }}>
        ...
        <div style={{ height:52, flexShrink:0, background:"#0d0d14", borderBottom:"1px solid #1e1e2e", display:"flex", alignItems:"center", padding:"0 10px 0 14px", gap:8 }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {topicId === "custom" && <button style={smallBtn(28)} onClick={() => setShowEditModal(true)} title="Bearbeiten">✏️</button>}
