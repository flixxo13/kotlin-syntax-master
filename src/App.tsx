...
export default function App() {
  ...
  const [activeExercise, setActiveExercise] = useState<{ topicId: string; conceptId: string; exerciseId?: string; deckId?: string } | null>(null);
  const { customTasks, reconcileDecks } = useLearningStore();
    ...
    if (activeExercise) {
      return (
        <ExerciseScreen 
          ...
          deckId={activeExercise.deckId}
          onBack={() => setActiveExercise(null)}
          onStartCustomTask={(taskId, deckId) => {
            const task = customTasks.find((t: any) => t.id === taskId);
            if (task) setActiveExercise({ topicId: 'custom', conceptId: task.conceptId, exerciseId: task.id, deckId });
          }} 
        />
      );
    }
        ...
        return (
          <LearnScreen 
            ...
            onStartCustomTask={(taskId, deckId) => {
              const task = customTasks.find(t => t.id === taskId);
              if (task) {
                setActiveExercise({ topicId: 'custom', conceptId: task.conceptId, exerciseId: task.id, deckId });
              }
            }}
            onGoToImport={() => setActiveTab('import')}
          />
